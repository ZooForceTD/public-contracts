...
import "./AccessoryMinteableERC721.sol";
...
contract AccessoryCore is AccessoryMinteableERC721, AccessControl, Ownable, IZooForceNFTCore {
    ...
    constructor(address _presale)
        ERC721Metadata("Accessory","ZFACCESSORY")
        public{
    ...
    function createZooForceNFTBatch(address _owner, uint256[4] calldata _natureCodes) external onlyCreator {
        _mintZooNFTDeploymentBatch(_owner, _natureCodes, nextTokenId);
        nextTokenId = nextTokenId + 4;
    }

    /// @notice Returns all the relevant information about a specific Accessory.
    /// @param _id The ID of the Accessory of interest.
    function getAccessory(uint256 _id)
        public
        view
        returns (uint256 birthMoment, uint256 natureCode){
        Accessory storage accessory = Accessories[_id - 1];

        birthMoment = uint256(accessory.birthMoment);
        natureCode = accessory.natureCode;
    }
